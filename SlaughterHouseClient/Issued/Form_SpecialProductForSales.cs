...
{
    public partial class Form_SpecialProductForSales : Form
    {
        ...
        public Form_SpecialProductForSales()
        {
            InitializeComponent();
            Load += Form_Load;
        ...
        private void LoadOrder()
        {
            try
            {
                using (var db = new SlaughterhouseEntities())
                {
                    ...
                    var coll = (from ord in order.orders_item
                                join p in db.products on ord.product_code equals p.product_code
                                select new
                                {
                                    ord.seq,
                    ...
                    gv.Columns[2].HeaderText = "จำนวนสั่ง";
                    gv.Columns[3].HeaderText = "น้ำหนักสั่ง";
                    gv.Columns[4].HeaderText = "จำนวนจ่าย";
                    gv.Columns[5].HeaderText = "น้ำหนักจ่าย";
            ...
            try
            {
                ...
                using (var db = new SlaughterhouseEntities())
                {
                    ...
                    var orderItems = db.orders_item.Where(p => p.order_no == orderNo
                                && p.product_code == barcode.product_code).SingleOrDefault();
                    if (orderItems == null)
                    {
                    ...
                    else
                    {
                        ...
                        stockNo = transport.transport_item.Select(p => p.stock_no).FirstOrDefault();
                        stockItem = transportItem;
                    }
                    ...
                    //    //add stock_item_running
                    //    stockItemRunning = new stock_item_running
                    //    {
                    //        doc_no = orderNo,
                    //        stock_no = stockNo,
                    ...
                    //        create_by = createBy
                    //    };
                    //    db.stock_item_running.Add(stockItemRunning);
