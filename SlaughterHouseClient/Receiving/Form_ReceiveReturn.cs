...
{
    public partial class Form_ReceiveReturn : Form
    {
        ...
        private RMIssuedInfo productIssued;
        ...
        private void LoadStock()
        {
            using (var db = new SlaughterhouseEntities())
            {
                ...
                && p.product_code == productIssued.product_code
                && p.lot_no == productIssued.lot_no
        ...
        private void btnProduct_Click(object sender, EventArgs e)
        {
            try
            {
                var frm = new Form_LookupProductIssued();
                //frm.ProductGroup = 9;//ชิ้นส่วนหลัก
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    //product_code = frm.ProductCode;
                    productIssued = frm.productIssuedSelected;
                    LoadProduct();
                    btnStart.Enabled = true;
                    //lot_no = "";
